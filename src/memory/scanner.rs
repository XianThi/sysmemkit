...
pub unsafe fn pattern_scan_local(start: usize, size: usize, pattern: &str) -> Option<usize> {
    let mut buffer = vec![0u8; size];

    unsafe { std::ptr::copy_nonoverlapping(start as *const u8, buffer.as_mut_ptr(), size) };
